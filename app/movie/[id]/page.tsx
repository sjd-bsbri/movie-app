...
// // تولید مسیرهای استاتیک برای هر فیلم
// export function generateStaticParams() {
//   return movies.map((movie) => ({
//     id: movie.id.toString(),
//   }));
...
// نوع پراپ‌ها با در نظر گرفتن ساختار جدید Next.js 15 اصلاح شد
type PageProps = {
  params: Promise<{
    id: string;
  }>;
...
// تولید مسیرهای استاتیک برای هر فیلم (بدون تغییر)
export function generateStaticParams() {
  return movies.map((movie) => ({
    id: movie.id.toString(),
  }));
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  // مقدار id با await از پراپ params استخراج می‌شود
...
// صفحه نمایش اطلاعات فیلم (اصلاح شده)
export default async function MovieDetailPage({ params }: PageProps) {
  // مقدار id با await از پراپ params استخراج می‌شود
  const { id } = await params;
  const movie = getMovieById(parseInt(id));
